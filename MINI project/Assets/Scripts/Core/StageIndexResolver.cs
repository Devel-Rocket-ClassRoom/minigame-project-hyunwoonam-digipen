...
namespace Tempt
{
    ...
    public static class StageIndexResolver
    {
        ...
        public static int FromFloor(int floor, WorldData world)
        {
            ...
            if (world.SafeZones != null)
            {
                for (int i = 0; i < world.SafeZones.Count; i++)
                {
                    ...
                    {
                        return SafeStageForIndex(safeZone.Index);
                    }
                }
            }
        ...
        public static int FromNode(FloorNode node)
        {
            ...
            return Mathf.Clamp(node.StageIndex, 1, ErosionSystem.MaxStage);
        }
        ...
        private static int SafeStageForIndex(int safeIndex)
        {
            if (safeIndex <= 0)
            {
                return 1;
            ...
            return Mathf.Clamp(safeIndex, 1, ErosionSystem.MaxStage);
        }
    }
}
