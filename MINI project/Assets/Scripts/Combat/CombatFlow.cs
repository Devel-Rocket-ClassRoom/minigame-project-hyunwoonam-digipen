...
namespace Tempt
{
    ...
    public sealed class CombatFlow
    {
        ...
        public void Tick()
        {
            ...
            switch (State)
            {
                ...
                case CombatState.Resolving:
                    ...
                    if (currentAction == null || elapsedTime >= currentAction.DurationSec)
                    {
                        ConsumeAndRoundCheck();
                    }
                    break;
                // Wave0refactor 2026-05-27 (F.2): CheckOutcome 호출은 ResolveAction 단일 위치에서만.
        ...
        {
            // 동작 요약:
            // - action.DurationSec = ActionTiming.Compute(action) (0.1s 최소 + 애니/이펙트 합산).
            // - 타입별 즉시 효과 적용:
            //   * Attack → DamageCalculator.ComputeAttack(actor, target) + actor.WorldUI 빨강 아이콘.
            ...
            action.DurationSec = ActionTiming.Compute(action);
