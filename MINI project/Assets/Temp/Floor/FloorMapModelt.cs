...
namespace Tempt
{
    ...
    /// 이어하기는 seed 재생성이 아니라 SaveSnapshott.FloorMap.Nodes를 통해 이 모델을 복원한다.
    /// </summary>
    public sealed class FloorMapModelt
    {
        ...
        public void MarkCleared(int nodeId)
        {
            ...
            //TODO: }
            if (!NodesById.TryGetValue(nodeId, out FloorNodet node)) //Wave0write
            { //Wave0write
                return; //Wave0write
            } //Wave0write
        ...
        public bool IsStageCleared(int stageIndex)
        {
            ...
            if (bossFloor <= 0 || !NodesByFloor.TryGetValue(bossFloor, out List<FloorNodet> nodes)) //Wave0write
            { //Wave0write
        ...
        { //Wave0write
            ...
            { //Wave0write
                ...
                if (NodesByFloor.TryGetValue(floor, out List<FloorNodet> nodes)) //Wave0write
                { //Wave0write
