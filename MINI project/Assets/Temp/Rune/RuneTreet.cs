...
namespace Tempt
{
    ...
    public sealed class RuneTreet
    {
        ...
        /// <summary>모든 노드(ID → RuneNodet, 조회용).</summary>
        ...
        public static RuneTreet BuildFromData(RuneClasst classId, IEnumerable<RuneDatat> allRunes)
        {
            ...
            // - AllNodes를 먼저 Id → RuneNodet 로 구성.
            // - 각 RuneDatat.RequiredRuneId 를 부모 키로 사용해
            //   AllNodes[RequiredRuneId].Next.Add(현재 노드) 로 자식 연결.
            // - 사이클 검사(BFS/DFS).
            //TODO: var tree = new RuneTreet { ClassId = classId, AllNodes = new Dictionary<int, RuneNodet>() };
            //TODO: foreach (var data in allRunes)
            //TODO: {
            //TODO:     if (data.ClassId != classId && data.RuneType != RuneNodeTypet.SubFragment) continue;
            //TODO:     var node = new RuneNodet { Data = data, Next = new List<RuneNodet>() };
            ...
            //TODO: }
            //TODO: return tree;
            var tree = new RuneTreet //Wave0write
            { //Wave0write
                ClassId = classId, //Wave0write
                AllNodes = new Dictionary<int, RuneNodet>(), //Wave0write
            }; //Wave0write
            ...
            foreach (RuneDatat data in allRunes) //Wave0write
            { //Wave0write
                if (data.ClassId != classId && data.RuneType != RuneNodeTypet.SubFragment) //Wave0write
                { //Wave0write
                    continue; //Wave0write
                } //Wave0write

                var node = new RuneNodet { Data = data, Next = new List<RuneNodet>() }; //Wave0write
            ...
            { //Wave0write
                ...
                if (parentId != 0 && tree.AllNodes.TryGetValue(parentId, out RuneNodet parent)) //Wave0write
                { //Wave0write
        ...
        public bool CanUnlock(RuneNodet node)
        {
            ...
            return AllNodes != null && AllNodes.TryGetValue(node.Data.RequiredRuneId, out RuneNodet parent) && parent.Unlocked; //Wave0write
        }
    }
}
