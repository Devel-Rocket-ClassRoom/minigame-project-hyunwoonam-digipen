...
namespace Tempt
{
    ...
    ///
    /// 흐름:
    ///   1) GameSystemManagert.EndCombat → UIManagert.ShowCombatReward(summary, overflowIds, onClose) 호출.
    ...
    /// </summary>
    public sealed class CombatRewardPaget : MonoBehaviour
    {
        /// <summary>페이지가 닫힐 때 호출되는 콜백(씬 전환 트리거).</summary>
        private Action onCloseCallback;

        /// <summary>현재 표시 중인 보상 요약.</summary>
        private NodeRewardSummaryt currentSummary;
        ...
        public void Show(NodeRewardSummaryt summary, List<int> overflowIds, Action onClose)
        {
            // 동작 요약:
        ...
        /// </summary>
        private void BindSummaryPanel(NodeRewardSummaryt summary)
        {
            // 동작 요약:
            ...
            //TODO: {
            //TODO:     ItemDatat data = GameSystemManagert.Instance.Data.Items[id];
        ...
        private void RefreshOverflowPanel()
        {
            ...
            //     * ItemDatat data = GameSystemManagert.Instance.Data.Items[itemId].
        ...
        {
            // 동작 요약:
            // - InventoryStatet inv = GameSystemManagert.Instance.CurrentRun.Player.Inventory.
            // - ItemDatat data = GameSystemManagert.Instance.Data.Items[itemId].
            // - data.IsStackable:
            //     * inv.TryAdd(itemId, 1) 성공 → pendingOverflowIds에서 itemId 하나 제거.
            //     * 실패 → 인벤토리 꽉 찼음 토스트 표시 (버리기 필요 안내).
            // - !data.IsStackable:
            //     * inv.TryAddEquip(new Itemt(data)) 성공 → 제거.
            //     * 실패 → 토스트.
            // - RefreshOverflowPanel().
            //TODO: var inv  = GameSystemManagert.Instance.CurrentRun.Player.Inventory;
            //TODO: var data = GameSystemManagert.Instance.Data.Items[itemId];
            //TODO: bool success;
            //TODO: if (data.Stackable)
            //TODO:     success = inv.TryAdd(itemId, 1);
            //TODO: else
            //TODO:     success = inv.TryAddEquip(new Itemt { Data = data, Enhancement = 0 });
