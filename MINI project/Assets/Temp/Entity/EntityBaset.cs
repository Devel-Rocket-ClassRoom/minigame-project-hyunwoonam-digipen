...
namespace Tempt
{
    ...
    /// 패시브 스킬은 CharacterBaset.SyncPassivesFromRunes()가 룬 해금 시 자동으로 채우며
    ...
    public abstract class EntityBaset : MonoBehaviour
    {
        ...
        /// </summary>
        public IReadOnlyList<Skillt> PassiveSkills => passiveSkills;

        private List<Skillt> passiveSkills = new List<Skillt>();
        ...
        public virtual void PrepareForCombat()
        {
            ...
            //TODO: if (ActiveSkills == null || ActiveSkills.Length != 2) ActiveSkills = new Skillt[2];
        ...
        /// <summary>
        /// 패시브 스킬 추가. CharacterBaset.SyncPassivesFromRunes()만 호출해야 한다.
        /// 외부에서 직접 호출 금지.
        /// </summary>
        internal void AddPassiveSkill(Skillt skill)
        {
            // 동작 요약:
            // - skill.Data.SkillType == Passive 검증.
            // - passiveSkills에 중복 없이 추가.
            //TODO: if (skill == null || skill.Data == null) return;
            //TODO: if (skill.Data.SkillType != SkillTypet.Passive) return;
            //TODO: if (!passiveSkills.Contains(skill)) passiveSkills.Add(skill);
            if (skill?.Data == null || skill.Data.SkillType != SkillTypet.Passive) //Wave0write
            { //Wave0write
                return; //Wave0write
            } //Wave0write
        ...
        protected void ApplyAllPassiveEffects()
        {
            ...
            //TODO: foreach (Skillt passive in passiveSkills)
            //TODO: {
            //TODO:     if (passive?.Data == null) continue;
            //TODO:     SkillDatat d = passive.Data;
            //TODO:     // DamageScale → ATK 보정, HealScale → HP 보정, ShieldScale → DEF 보정 등
            //TODO:     // 실제 매핑은 SkillDatat.EffectFormula 필드 확정 후 분기 처리
            //TODO:     // 예: if (d.DamageScale > 0f) Stats.ApplyPassiveBonus(StatTypet.ATK, (int)(Stats.ATK * d.DamageScale));
            //TODO: }
            if (Stats == null) //Wave0write
            { //Wave0write
                return; //Wave0write
            ...
            foreach (Skillt passive in passiveSkills) //Wave0write
            { //Wave0write
                SkillDatat data = passive?.Data; //Wave0write
                ...
                { //Wave0write
                    Stats.ApplyPassiveBonus(StatTypet.ATK, Mathf.RoundToInt(Stats.BaseATK * data.DamageScale)); //Wave0write
                } //Wave0write
                if (data.HealScale > 0f) //Wave0write
                { //Wave0write
                    Stats.ApplyPassiveBonus(StatTypet.HP, Mathf.RoundToInt(Stats.BaseMaxHP * data.HealScale)); //Wave0write
                } //Wave0write
                if (data.ShieldScale > 0f) //Wave0write
                { //Wave0write
                    Stats.ApplyPassiveBonus(StatTypet.DEF, Mathf.RoundToInt(Stats.BaseDEF * data.ShieldScale)); //Wave0write
                } //Wave0write
            } //Wave0write
        }
    }
