...
namespace Tempt
{
    ...
    /// 드랍은 DropTableId를 통해 DataManagert.ResolveDrops()로 결정.
    /// 각각의 몬스터 코드(Monster1t 등)에서 DropTableId를 포함한 스탯/스킬/애니메이션을 재정의한다.
    /// </summary>
    public abstract class MonsterBaset : EntityBaset
    {
        ...
        /// <summary>
        /// 드랍 테이블 ID. DataManagert.ResolveDrops(DropTableId)로 실제 드랍 결정.
        /// 0이면 드랍 없음.
        /// 각 몬스터 코드(Monster1t 등)에서 재정의(override 또는 InitializeFromData 복사).
        /// </summary>
        ...
        public void InitializeFromData(MonsterDatat data, float erosionMultiplier)
        {
            ...
            //TODO: // 스킬 매핑
            //TODO: ActiveSkills = new System.Collections.Generic.List<Skillt>();
            //TODO: foreach (int sid in data.SkillIds)
            //TODO:     ActiveSkills.Add(new Skillt(GameSystemManagert.Instance.Data.Skills[sid]));
            ...
            if (data.SkillIds != null && GameSystemManagert.TryGetInstance(out GameSystemManagert gsm)) //Wave0write
            { //Wave0write
                for (int i = 0; i < data.SkillIds.Count && i < ActiveSkills.Length; i++) //Wave0write
                { //Wave0write
                    if (gsm.Data.Skills.TryGetValue(data.SkillIds[i], out SkillDatat skillData)) //Wave0write
                    { //Wave0write
                        ActiveSkills[i] = new Skillt(skillData); //Wave0write
                    } //Wave0write
                } //Wave0write
            } //Wave0write

            ActionWeights = data.ActionWeights ?? new ActionWeightTablet { Attack = 80, Skill = 0, Defend = 20 }; //Wave0write
        ...
        public void ApplyErosionVisual()
        {
            ...
            //TODO: if (sr != null)
            //TODO: {
            //TODO:     MonsterDatat data = GameSystemManagert.Instance.Data.Monsters[MonsterDataId];
        ...
        /// 드랍 아이템은 DataManagert.ResolveDrops(DropTableId)로 확률 계산.
        /// </summary>
        public NodeRewardContributiont GetRewardContribution()
        {
            // 동작 요약:
            // - DataManagert data = GameSystemManagert.Instance.Data.
            // - List<DroppedItemStackt> drops = data.ResolveDrops(DropTableId).
            // - DroppedItemIds 목록 = drops.SelectMany(d => Enumerable.Repeat(d.ItemId, d.Count)).
            // - NodeRewardContributiont { Exp = RewardExp, Gold = RewardGold, DroppedItemIds } 반환.
            //TODO: DataManagert dataMgr = GameSystemManagert.Instance.Data;
            //TODO: List<DroppedItemStackt> drops = dataMgr.ResolveDrops(DropTableId);
            //TODO: var itemIds = new List<int>();
            //TODO: foreach (var stack in drops)
            //TODO:     for (int i = 0; i < stack.Count; i++) itemIds.Add(stack.ItemId);
            //TODO: return new NodeRewardContributiont { Exp = RewardExp, Gold = RewardGold, DroppedItemIds = itemIds };
            var itemIds = new List<int>(); //Wave0write
            if (GameSystemManagert.TryGetInstance(out GameSystemManagert gsm)) //Wave0write
            { //Wave0write
                List<DroppedItemStackt> drops = gsm.Data.ResolveDrops(DropTableId); //Wave0write
                foreach (DroppedItemStackt stack in drops) //Wave0write
                { //Wave0write
            ...
            return new NodeRewardContributiont { Exp = RewardExp, Gold = RewardGold, DroppedItemIds = itemIds }; //Wave0write
        }
    }
