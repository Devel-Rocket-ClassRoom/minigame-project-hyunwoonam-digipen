namespace Tempt
{
    ...
    public abstract class CharacterBaset : EntityBaset
    {
        ...
        public void GainExp(int amount)
        {
            ...
            //TODO: GameSystemManagert.Instance.Events.RaisePlayerExpChanged(CurrentExp, RequiredExp);
        ...
        protected virtual void LevelUp()
        {
            ...
            // - 룬 포인트 적립(Player는 PlayerRuneStatet.AddPoint; Companion은 CompanionRuneStatet 자동 해금).
            // - SyncPassivesFromRunes() 호출 → 패시브 목록 갱신.
            // - 이벤트 발행.
            //TODO: CurrentExp -= RequiredExp;
            //TODO: Level += 1;
            //TODO: BalanceDatat balance = GameSystemManagert.Instance.Data.Balance;
            //TODO: RequiredExp = (Level < balance.ExpToNextLevel.Count) ? balance.ExpToNextLevel[Level] : int.MaxValue;
            //TODO: SyncPassivesFromRunes(GameSystemManagert.Instance.Data);
        ...
        /// 플레이어(PlayerRuneStatet 사용)와 동료(CompanionRuneStatet 사용) 모두 이 흐름을 따른다.
        /// </summary>
        /// <param name="data">DataManagert 참조. 스킬 데이터 조회용.</param>
        public void SyncPassivesFromRunes(DataManagert data)
        {
            // 동작 요약:
            // - ClearPassiveSkills().
            // - 해금된 룬 노드 목록 조회(파생 클래스가 GetUnlockedRuneNodeIds()로 제공).
            // - 각 RuneDatat 조회: EffectType == UnlockSkill → EffectValue를 skillId로 변환.
            ...
            //TODO: {
            //TODO:     if (!data.Runes.TryGetValue(nodeId, out RuneDatat runeData)) continue;
            //TODO:     if (runeData.EffectType != RuneEffectTypet.UnlockSkill) continue;
            //TODO:     int skillId = (int)runeData.EffectValue;
            //TODO:     if (!data.Skills.TryGetValue(skillId, out SkillDatat skillData)) continue;
            //TODO:     if (skillData.SkillType != SkillTypet.Passive) continue;
            //TODO:     Skillt passive = new Skillt(skillData);
            ...
            { //Wave0write
                if (!data.Runes.TryGetValue(nodeId, out RuneDatat runeData) || runeData.EffectType != RuneEffectTypet.UnlockSkill) //Wave0write
                { //Wave0write
                    continue; //Wave0write
                } //Wave0write

                int skillId = (int)runeData.EffectValue; //Wave0write
                if (data.Skills.TryGetValue(skillId, out SkillDatat skillData) && skillData.SkillType == SkillTypet.Passive) //Wave0write
                { //Wave0write
                    AddPassiveSkill(new Skillt(skillData)); //Wave0write
                } //Wave0write
            } //Wave0write
        }
        ...
        /// Player는 PlayerRuneStatet.UnlockedNodeIds, Companion은 CompanionRuneStatet 기반.
        /// </summary>
        ...
        { //Wave0write
            if (GameSystemManagert.TryGetInstance(out GameSystemManagert gsm) && gsm.Data?.Balance?.ExpToNextLevel != null && level >= 0 && level < gsm.Data.Balance.ExpToNextLevel.Count) //Wave0write
            { //Wave0write
